...
//		}
		...
		// 논리And는 둘 다 true일 때만 true를 반환한다.
		if (num%2==0 && num>=10) {
		...
		/* 조건 검사 후 실행할 문장이 1개인 경우에는 중괄호를 생략할 수 있다. 단, 실행문이
		2개 이상이라면 한 문장만 if문에 포함되므로 반드시 중괄호를 기술해야 한다. */
		if (num%2 == 0) {
			System.out.println("\n" + num + "은 짝수입니다.");
		}
		if (num%2 != 0) {
		...
		/* 아스키코드를 알고 있을 때의 구현 방법. 문자 0과 9는 아스키코드 48, 57에
		해당하므로 구간 내에 있는지 확인하면 된다. */
		if (asciicode>=48 && asciicode<=57) {
			System.out.println("입력한 문자는 숫자입니다.");
		} else {
			System.out.println("입력한 문자는 숫자가 아닙니다.");
		}
		/* 아스키코드를 모를 때 구현하는 방법으로 문자 '0'과 '9'는 메모리에 저장시
