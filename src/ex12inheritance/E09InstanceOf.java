...
/* instanceof 연산자 : 인스턴스 변수가 어떤 타입의 변수인지를 판단하는 연산자로 형변환
(즉 상속관계가 있으면)이 가능하면 true를 반환한다.
- 객체간에 형변환이 되려면 반드시 두 클래스간에 상속관계가 있어야 한다.
- 부모타입의 참조변수로 자식인스턴스를 참조할 수 있으므로 매개변수를 부모타입으로 만들면
자식인스턴스를 인수로 받을 수 있다.
- 매개변수로 전달되는 인수는 부모타입으로 자동형변환(업캐스팅)되므로 정상적인 사용을 위해
강제형변환(다운캐스팅) 해야하는데 이때 확인을 위해 사용된다. */
class Box {
	public void boxWrap() {
...
public class E09InstanceOf {
	/* - 메소드 정의시 부모타입의 매개변수로 설정하면 자식타입의 인스턴스를 한꺼번에 받을 수
	있다.
	- Box타입으로 인수를 받게 되면 자동형변환(업캐스팅)되므로 자식클래스의 멤버메소드를
	호출할 수 없게 된다.
	...
	/* 만약 부모타입의 매개변수로 자식타입의 인수를 받을 수 없다면 아래와 같이 각각의 타입에
	따른 모든 메소드를 정의해야 한다.
	...
	상속관계가 더 깊어진다면 메소드는 점점 늘어나게 되어 코드는 더욱 더 복잡해지게 될 것이다.
	...
	public static void main(String[] args) {
		...
		/* 각 자식 타입의 인수를 부모타입의 매개변수로 전달한다. Box가 최상위 부모이므로
		하위의 모든 인수를 받을 수 있다. 이는 상속관계가 있으므로 가능하다. */
		wrapBox(box1);
		wrapBox(box2);
		wrapBox(box3);
		
		/* String인스턴스는 Box와는 아무런 관계가 없으므로 인수로 전달할 수 없다. 즉
		상속관계가 성립하지 않는다. */
