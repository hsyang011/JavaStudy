...
// 부모클래스와 다른 패키지에 선언되었으므로 import해야 한다.
...
class Seoul extends Korea {
	...
	/* 4개의 접근지정자를 통해 정의한 메소드를 호출한다. 단 부모클래스가 다른 패키지에
	선언되었으므로 default, private멤버는 호출할 수 없다. */
	public void callMethod() {
		super.publicMethod("callMethod->");
		super.protectedMethod("callMethod->");
//		super.defaultMethod(); // 에러
//		super.privateMethod(); // 발생
		/* 이와 같이 직접 접근할 수 없는 메소드는 접근 가능한 멤버메소드를 통해
		간접적으로 호출하면 된다. */
	}
}
