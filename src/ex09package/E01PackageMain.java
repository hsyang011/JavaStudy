...
/* 임포트 선언부 : 현재 작성중인 클래스에서 다른 패키지에 있는 클래스를 가져다 쓰기 위한
선언으로, 사용할 클래스의 위치를 지정하는 것으로 생각하면 된다. */
...
public class E01PackageMain {
	...
	public static void main(String[] args) {
		...
		/* static으로 선언된 메소드는 new를 통해 인스턴스 생성없이 클래스명으로 직접
		메소드를 호출할 수 있다. Java에서 제공하는 클래스 중 대표적으로 Math를 예로 들
		수 있다. 난수생성을 위해 별도의 인스턴스 생성없이 Math.random()을 통해 난수를
		생성한다. */
