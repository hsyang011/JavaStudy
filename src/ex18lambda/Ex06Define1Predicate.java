...
/* Java에서 제공하는 함수형 인터페이스에는 아래 4가지 정도가 있다. 차이점이라면 매개변수의
유무와 리턴값 정도이다.
...
public class Ex06Define1Predicate {
	...
	public static void main(String[] args) {
		...
		/* Predicate<T>
		- 사전적의미 : ~에 근거를 두다
		- 매개변수와 boolean리턴값이 있는 추상메소드 test()가 정의되어 있다.
		...
		인터페이스에 정의된 test()를 오버라이딩하여 아래와 같은 람다식을 정의한다.
		매개변수로 Human객체를 전달하고, getter인 getGender()를 통해 성별이 '남'인
		경우 true를 반환하도록 정의되어 있다. */
		...
		/* 인수로 사용된 4개의 객체를 저장한 List컬렉션을 생성한다. 단 해당 컬렉션은
		값을 변경할 수는 없고 참조만 가능하다. */
		List<Human> list1 = Arrays.asList(p1, p2, p3, p4);
		
		/* Predicate<Human> aaa =
			(Human h) -> { h.getGender().equals("남"); };
		이와 같은 람다식을 정의한 후 참조변수인 aaa를 전달하는 것과 아래처럼 람다식 자체를
		매개변수로 작성하는 것은 동일하다. */
		double maleAvg = avg((Human h) -> h.getGender().equals("남"), list1);
		System.out.println("남자 평균:" + maleAvg);
	}
	
	/* 첫번째 매개변수는 성별이 "남"이면 true를 반환하는 람다식이고 두번째 매개변수는
	4개의 Human객체가 저장된 List컬렉션이다. */
