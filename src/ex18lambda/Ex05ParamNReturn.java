...
// 매개변수는 있으나 반환값이 없는 추상메소드 : 두 개의 문자열을 전달받아 연결해서 출력한다.
@FunctionalInterface
interface StringConcat {
	public void makeString(String s1, String s2);
}

/* 매개변수는 없으나 반환값은 있음 : 0~99사이의 난수를 생성하여 반환한다. */
@FunctionalInterface
interface RandomNumber {
	public int randomNum();
}

/* 매개변수와 반환값 둘 다 있음 : 매개변수로 전달된 두 정수만큼의 구구단을 2차원배열로
 * 반환한다. */
@FunctionalInterface
...
/* 매개변수도 없고 반환값도 없음 : 단순히 메뉴만 출력하는 기능을 가진다. */
@FunctionalInterface
interface MenuPrint {
...
public class Ex05ParamNReturn {
	...
	public static void main(String[] args) {
		...
		String s1 = "안녕하세요?";
		String s2 = "람다식입니다.";
		/* 2개의 매개변수는 String으로 추상메소드를 통해 유추할 수 있으므로 람다식에서는
		생략할 수 있다. 메소드명을 제거한 후 괄호 사이에 화살표를 삽입하여 람다식을
		정의한다. 호출시에는 부모인터페이스의 추상메소드인 makeString()을 이용하면 된다.
		 */
		StringConcat sc = (a, b) -> {
		...
		System.out.println("\n### 람다식2");
		/* 매개변수가 없는 람다식을 정의해야 하므로 빈 소괄호를 사용한다. 난수를 생성해서
		반환하는 기능을 정의하였다. */
		...
		/* 람다식 호출시 두 정수를 인수로 전달한 후 2차원배열의 반환값을 받는다. */
