...
/* 이와 같은 추상클래스를 아래에 있는 인터페이스로 변경할 수 있다. 인터페이스에 정의된 모든
메소드는 public abstract가 추가된다.
abstract class ISchool1 {
	abstract void studyLambda(String str);
} */

/* 인터페이스 정의 : 인터페이스에는 추상메소드만 정의할 수 있다. 구현부가 없는 메소드이므로
...
부모인터페이스의 추상메소드를 재정의해야 한다. 이렇게 가려지게 된다. */
...
public class Ex01NormalClass {
	...
	public static void main(String[] args) {
		/* 단지 메소드 하나를 정의해서 사용하고 싶은데 인터페이스, 상속, 오버라이딩, 객체
		생성까지의 과정을 거쳐야 하므로 너무 복잡하다.
		
		여기서는 부모타입의 참조변수를 토앻 객체를 생성한 후 오버라이딩된 자식쪽의 메소드를
		호출하고 있다. */
