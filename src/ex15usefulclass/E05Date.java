...
public class E05Date {
	...
	/* Calendar클래스를 통해 얻은 인스턴스를 초로 변환한 후 매개변수로 전달된 날짜서식으로
	변경한 String을 반환한다. */
	public static String myDateFormat(Calendar c, String formatStr) {
		/* getTime() : 서버의 표준시인 1970-01-01부터 현재까지의 시간을 초단위로
		반환해준다. 이를 타임스탬프라고 한다. */
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		/* Calendar클래스는 대표적인 싱글톤 객체이므로 new로 인스턴스를 생성할 수
		없고, 유틸리티 메소드를 통해 참조값을 얻어와야 한다. */
		...
		/* 현재 날짜를 기준으로 전/후를 구하기 위해 add()메소드를 사용한다. 만약 음수를
		주면 과거의 날짜를 구할 수 있다. */
...
//		System.out.println(today + " " + future);
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			/* 매개변수를 통해 전달된 2개의 문자열은 날짜의 서식이 서로 다르다. 따라서
			각 날짜의 서식을 명확히 지정해야지만 날짜를 인식하여 연산할 수 있다.
			...
			/* getTime()으로 얻어오는 시간은 13자리의 정수값인데 10자리까지는 순수한
			초(sec)이고, 11~13자리는 밀리세컨즈를 의미한다. 즉 100으로 나눠서 뒤의
			3자리를 제거하면 순수한 초가 반환된다. 또한 하루는 60*60*24 즉 86400초
			이므로 아래와 같이 남은 날짜를 구할 수 있다. */
