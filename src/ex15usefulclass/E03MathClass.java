...
/* Java의 규칙 : java.lang패키지 하위의 모든 클래스는 자동으로 import된다. 또한
정의하는 모든 클래스는 Object를 상속받게 되어 있다.
따라서 java.lang패키지의 모든 클래스는 별도의 선언없이 사용할 수 있고, Object에서
제공하는 메소드는 오버라이딩이 가능하다. */
...
public class E03MathClass extends Object {
	...
	- 수학과 관련된 일련의 작업들을 처리하는 메소드가 정의되어 있다.
	- 메소드는 static으로 선언되어 있어 인스턴스 생성없이 클래스명으로 호출 가능하다. */
	public static void main(String[] args) {
		...
		/* round의 경우 가장 가까운 정수쪽으로 올림/버림을 결정한다. x.5 이상이면
		올림처리, 미만이면 버림처리를 한다. 따라서 3.4xxx라면 버림처리를 하게 된다. */
		...
		System.out.println("Math.max:" + Math.min(100, 99));
		
		/* 0~1사이의 실수값을 반환한다. 따라서 정수값을 만들고 싶을 때는 특정한 값을
		곱해주면 된다. */
