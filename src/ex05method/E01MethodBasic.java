...
/*
메소드(함수)의 규칙
- Java에서 main메소드는 무조건 public static void로 선언한다.
- static으로 선언된 메소드는 static으로 선언된 메소드만 호출할 수 있다.
- 메소드 호출 후 반환값은 호출한 위치로 반환된다. 이때 메소드는 메모리에서 소멸된다.
- 반환값이 없는 void형 메소드도 실행이 완료되면 호출한 위치로 실행의 흐름이 돌아온다.
 */
public class E01MethodBasic {
	
	/*
	메소드1 : 반환값 O, 매개변수 O 형태로 정의
	=> 매개변수로 2개의 정수를 전달받아 합의 결과를 반환한다. 반환값은 호출한 지점으로
	반환된다.
	 */
	...
	/*
	메소드2 : 반환값 X, 매개변수X 형태로 정의
	=> 전달받는 값 없이 이름을 입력한 후 namePrint()메소드를 호출한다. 이와 같이 반환값이
	없는 경우에는 반드시 void를 명시해야 한다.
	 */
	...
	// main메소드는 Java에서 출발점(Entry point)의 역할을 한다.
	public static void main(String[] args) {
		/*
		메소드를 호출할 때는 함수의 원형을 그대로 기술하면 된다. 매개변수가 있다면 갯수만큼
		포함한다. 갯수가 다르거나 타입이 틀리면 에러가 발생한다.
		 */
