...
public class QuFindPrimeNumberT {
	...
	public static boolean isPrimeNumber(int n) {
		...
		/* 만약 정수가 7이라면 1과 7로는 무조건 나누어 떨어지므로 소수감사를 위해 2~6까지만
		나눠보면 된다. 따라서 정수 n이라면 2부터 n-1까지만 확인하면 된다. */
		for (int j=2; j<n; j++) {
			/* 만약 나누어 떨어지는 것이 발견되었다면 이미 소수가 아니므로 뒷부분은 확인할
			필요가 없다. 따라서 즉시 false를 반환하여 메소드를 종료한다. */
			if (n%j == 0) {
				return false;
			}
		}
