...
/* StringBuffer클래스 : String클래스는 기존 문자열에 새로운 문자열을 추가하면 새롭게 생성된
메모리에 문자열을 저장한다. 기존 메모리가 소멸되고 새로운 메모리가 생성되는 낭비를 막기 위해,
문자열의 변경이 많은 경우에는 StringBuffer클래스를 사용하는 것이 좋다. 해당 클래스는 기존
...
public class E03StringBuilderBuffer {
	...
	public static void main(String[] args) {
		...
		/* 더블쿼테이션으로 동일한 문자열 인스턴스를 생성했으므로 동일한 주소값을 가진다. */
		...
		/* String클래스는 문자열의 변경이 있는 경우 기존 메모리를 소멸하고 새로운 메모리를
		할당한다. */
		str1 = str1 + "&Spring";
		if (str1 == str2) {
		...
		/* StringBuffer클래스는 문자열을 저장하기 위해 기본메모리를 16으로 할당한다. 차후
		저장할 공간이 부족하면 자동으로 메모리를 확장한다. */
