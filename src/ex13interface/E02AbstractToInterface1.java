...
해당 클래스는 추상메소드를 포함하고 있으므로 하위에서 상속받게 되면 반드시 추상메소드를
오버라이딩 해야 한다. */
...
/* DTO(Data Transfer Object) : 데이터를 저장하고 전송하기 위한 용도의 객체를
말한다. VO(Value Object)로 표현하기도 한다. 해당 프로그램에서는 이름과 주민번호를
저장하는 용도로 사용된다. */
...
// 제공된 추상클래스를 상속하여 한 사람의 정보를 저장하는 기능과 검색의 기능을 구현한 클래스
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	...
	/* 상속을 통해 추상메소드를 포함하게 되므로 반드시 아래와 같이 오버라이딩 해야 한다.
	이는 필수사항이며 하지 않을 경우 에러가 발생한다. */
	@Override
