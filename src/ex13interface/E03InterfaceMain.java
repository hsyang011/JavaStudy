...
- 인터페이스는 생성자를 선안할 수 없다. */
interface MyInterface1 {
//	public MyInterface1() {}
	/* 멤버상수 : 인터페이스에 선언되는 변수는 상수로 선언된다. 따라서 반드시 대문자로
	작성해야 한다. */
	// public static final은 생략하는 것이 좋다.
	public static final int MAX_INT = Integer.MAX_VALUE;
	double MAX_DBL = Double.MAX_VALUE;
	
	/* 멤버메소드 : 메소드는 무조건 추상메소드로 선언된다. */
...
/* 인터페이스는 콤마를 이용해서 여러개의 인터페이스를 "구현"할 수 있다. 또한 "상속"과
"구현"을 동시에 사용할 수도 있다. */
class MyClass extends SimpleClass implements MyInterface1, MyInterface2 {
	...
	/* SimpleClass클래스의 멤버메소드는 필수사항이 아니므로 필요한 경우에만 오버라이딩
	하면 된다. */
}
