...
public class E03CharBooleanType {
	...
	public static void main(String[] args) {
		/*
		char형 : 하나의 문자를 저장할 수 있는 자료형으로 값을 저장할 때 ''(Single
			Quotation)으로 감싸서 표현한다. 만약 ""(Double Quotation)으로 감싸면
			문자열로 인식하므로 String으로 선언해야 한다.
		 */
		char ch1 = '가';
...
//		char chStr = '가나다라';
		...
		/*
		아스키코드 : 1byte로 표현할 수 있는 문자로 영문, 숫자를 10진수로 정의한 것을
			말한다.
			ex) A => 65, a => 97로 표현된다.
		유니코드 : 1byte로 표현할 수 없는 문자로 영문과 숫자를 제외한 모든 문자
		(한글, 한자 등)를 2byte로 표현한다. 보통 숫자가 크기 때문에 16진수로 표현한다.
		 */
		...
		/*
		boolean타입 : true 혹은 false 두 가지의 값만 가질 수 있는 자료형으로
			산술연산(+,-), 비교연산(<,<=)에서는 사용할 수 없고, 논리연산에서만 사용할
			수 있다.
		 */
		boolean bn1 = true;
		boolean bn2 = false;
		...
		&&(논리And) : 엠퍼센트를 사용하고 양쪽 모두 참일때만 참을 반환한다. 그 외에는
			모두 거짓을 반환한다.
		||(논리Or) : 파이프를 사용하고, 둘 중 하나만 차밍어도 참을 반환한다. 둘 다 거짓일
			때만 거짓을 반환한다.
		 */
		// T && F => false를 반환
		boolean bn3 = bn1 && bn2;
