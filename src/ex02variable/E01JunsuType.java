...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		/*
		100/200의 결과는 0이 나오게 된다. Java에서는 정수와 정수를 연산하면 정수의
		결과가 반환된다. 만약 실수의 결과를 반환받고 싶다면 형변환(Type Casting)을
		해야 한다.
		 */
		result1 = int1 / int2;
		System.out.println("int1 / int2 = " + result1);
		...
		- byte, short타입은 게임 캐릭터의 움직임이나 음원파일 등을 표현할 때 주로
			사용된다.
		 */
		...
		- 하지만 큰 자료형과 작은 자료형을 연산하면 큰 자료형으로 자동형변환 된다. 이유는
			데이터의 손실(오차)이 있을 수 있기 때문이다.
		- 단, byte와 short형은 예외적인 상황으로 생각하면 된다. CPU가 int형의 연산에
			최적화되어있기 때문이다.
		 */
		...
		/*
		자바컴파일러는 정수형 상수를 기본적으로 int형으로 간주하기 때문에 아래 정수를 메모리에
		로드하는 순간 에러가 발생한다. 이때 접미사(L 혹은 l)을 붙여서 int형의 자료가 아닌
		long형의 자료임을 컴파일러에게 알려줘야 한다. 이와 같이 할당을 위해 입력되는 숫자도
		자료형을 기반으로 메모리에 로드되는데 이를 가리켜 리터럴(literal)이라고 한다.
		 */
//		long long3 = 2200000000; // 에러발생
		long long4 = 2200000000L;
		long long5 = 3300000000l;
