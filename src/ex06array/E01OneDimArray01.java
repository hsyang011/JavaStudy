...
사용한다.
- 자바에서는 배열을 인스턴스형태로 생성하므로 new를 사용한다.
...
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		위해 일반적으로 반복문(for)를 사용한다.
		
		배열명.length : 해당 배열의 크기를 반환한다. 배열의 크기가 변경되는 경우 유연하게
		대처할 수 있다. */
		...
		/* 인스턴스 배열 : 기본자료형을 저장하는 배열이 아닌 인스턴스의 참조값을 저장할
		목적으로 생성한 배열을 말한다. 사용법은 동일하다. */
		...
		/* 배열의 선언과 동시에 초기화하는 경우 자바컴파일러는 초기값의 갯수를 통해 배열의
		크기를 결정할 수 있다. 따라서 이런 경우에는 크기는 생략하도록 약속되어있다. */
		System.out.println("방법2 : 선언과 동시에 초기화");
//		int[] initArr2 = new int[3] { 1, 20, 300 }; // 에러발생
		int[] initArr2 = new int[] { 1, 20, 300 };
		
		/* 배열 원소의 갯수를 통해 배열을 생성한 후 초기화까지 동시에 진행한다. 이 경우
		new를 생략할 수 있다. */
