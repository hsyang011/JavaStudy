...
class MessageSendingThread extends Thread {
	...
	public void run() {
		...
		for (int i=1; i<=1000; i++) {
			...
			실행중 잠깐씩 블록상태로 전환되어 우선순위가 낮은 쓰레드도 실행될 수 있는 기회가
			생기게 된다. */
			try {
				sleep(1);
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		우선순위를 부여받게 된다. 이때는 CPU가 알아서 작업을 분배한다. */
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		/* 2단계 실행 : Thread클래스의 정적상수를 이용해서 우선순위를 부여한다. 순서대로
		10, 5, 1의 우선순위를 부여하게 된다. 상수 대신 정수로 부여해도 된다. */
		MessageSendingThread tr1 = new MessageSendingThread("첫번째",
				Thread.MAX_PRIORITY);
		MessageSendingThread tr2 = new MessageSendingThread("두번째",
