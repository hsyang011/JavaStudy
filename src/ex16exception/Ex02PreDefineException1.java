...
public class Ex02PreDefineException1 {
	...
	public static void main(String[] args) {
		...
		JVM(자바가상머신)은 해당 예외에 관련된 객체를 만들어 프로그램쪽으로 전달한다.
		이때 catch절에서 예외객체를 잡아주는 것을 "예외처리"라고 한다.
		만약 예외객체를 잡아주지 못하면 프로그램은 비정상적인 종료로 이어지게 된다. */
		try {
			intArr[0] = 100;
			...
			/* 예외가 발생되는 지점
			해당 코드가 실행될 때 예외가 발생하게 되고 JVM이 예외객체를 생성해서
			프로그램으로 던진다.(throw) */
			intArr[2] = 300;
			System.out.println("앗~이방은?" + intArr[2]);
		} catch (ArrayIndexOutOfBoundsException e) {
			/* JVM이 throw한 예외객체를 catch절에서 잡아 처리한다. 그러면 예외가
			발생되어도 프로그램의 실행은 유지된다. */
