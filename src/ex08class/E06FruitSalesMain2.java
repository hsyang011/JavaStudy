...
/* 첫번째 버전은 각 클래스의 멤버변수가 고정된 초기값으로 선언되어 상태가 다른 인스턴스를
생성할 수 없다는 단점이 있었다. 두번째 버전에서는 초기화메소드를 추가하여 다양한 초기값을 가진
인스턴스를 생성할 수 있다. */
//과일판매자를 추상화한 클래스
class FruitSeller2 {
	// 멤버변수
	...
	int apple_price;
	/* 사과의 단가를 표현하는 멤버변수가 상수로 선언되면 선언과 동시에 초기화해야 한다. 또한
	초깃값이 없는 상태로 선언 자체가 불가능하다. 따라서 기존의 상수를 일반 변수로 변경해야
	한다. */

	// 멤버메소드
	/* 초기화메소드를 선언하여 객체 생성 후 다양한 형태의 초기화가 가능하다. */
...
public class E06FruitSalesMain2 {
	...
	public static void main(String[] args) {
		/* 초기화메소드가 정의되었으므로 상태가 다른 객체를 생성할 수 있다. */
