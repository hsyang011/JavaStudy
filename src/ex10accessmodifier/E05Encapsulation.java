...
/* 캡슐화(Encapsulation) : 여러가지 업무 로직을 하나로 묶는다는 의미와, 업무의
순서까지 고려한 형태의 로직을 구성한다는 의미를 가지고 있다. 즉 관련있는 로직을 하나로
묶어주는 것을 말한다. */
...
/* 업무의 순서를 고려하여 관련된 메소드를 하나의 클래스로 정의한다. 차후에는 별도의 분석
작업없이 해당 클래스의 doProcess()메소드만 호출하면 업무는 차질없이 진행될 것이다. */
class EncapsulLogic {
...
public class E05Encapsulation {
	...
	public static void main(String[] args) {
		...
		순서를 지키지 않은 상태에서도 실행이 가능한 코드가 된다. 즉 업무에 차질이
		발생될 수 있다. */
		...
		autoLogin.doAutoLogin(); // 이벤트페이지 이동
		congratulationPoint.doCongratulationPoint(); // 자동로그인
		firstLoginEvent.doFirstLoginEvent(); // 축하포인트 지급
		
		System.out.println("==============================");
		
		System.out.println("캡슐화 이후 코드");
		// 아래 메소드 호출을 통해 업무를 순서에 맞게 진행할 수 있다.
