...
class FruitSeller6 {
	/* 멤버변수에 private접근지정자를 추가한다. 이렇게 하면 클래스 내부에서만 접근할 수
	있고 외부에서의 접근은 원천적으로 차단할 수 있다. */
...
public class E03InfomationHiding {
	...
	public static void main(String[] args) {
		...
		FruitBuyer6 buyer = new FruitBuyer6();
		...
		//////////////////////////////////////////////////////
		/* 멤버변수를 private으로 선어하면 클래스 외부에서는 접근이 불가능하므로 아래
		코드는 에러가 발생하게 된다. (이 부분은 주석으로 처리한다.)
		즉, 멤버변수의 정보은닉을 통해 비정상적인 접근을 차단하고 멤버메소드를 통해서만
		구매가 이루어질 수 있도록 구현해야 한다. */
		
		// 각 판매자에게 1000원을 지불한다.
		buyer.buyApple(seller1, 1000); // 단가가 1000원 이므로 1개 구매 가능
		buyer.buyApple(seller2, 1000); // 단가가 500원 이므로 2개 구매 가능
