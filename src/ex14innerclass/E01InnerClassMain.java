...
/* 내부클래스(Inner class) : 클래스 안에 정의된 클래스로 static이 없는 클래스를
말한다.
...
class Unit {
	...
	public Unit(String name, int healthPoint) {
		...
		// 인스턴스형 멤버변수 객체 생성
		marine = new Marine();
		// Medic인스턴스 생서은 주석으로 처리하고 외부에서 생성한다.
//		this.medic = medic;
	}
...
public class E01InnerClassMain {
	...
	public static void main(String[] args) {
		...
		/* 외부클래스가 아닌 다른클래스에서는 내부멤버클래스가 보이지 않으므로 직접
		인스턴스를 생성할 수 없다. */
//		Marine m = new Marine(); // 에러발생
		
		/* 외부클래스의 인스턴스를 먼저 생성한 후 내부클래스의 인스턴스를 생성해야 한다.
		Marine은 Unit클래스의 생성자에서 이미 인스턴스 생성이 되어 있으므로 아래와 같이
		사용할 수 있다. */
		Unit.Marine marine = new Unit("마린", 50).marine;
		marine.attack();
		
		/* Medic의 경우 Unit의 생성자에서 인스턴스 생성이 되지 않았으므로 아래와 같이
		new를 통해 생성해야 한다. 이때에도 Unit인스턴스를 통해 생성해야 하므로 new를
		두 번 사용해서 인스턴스를 생성한다. */
