...
- 멤버변수에 static키워드를 붙이면 정적변수가 된다.
- 정적변수는 JVM(자바가상머신)에 의해 프로그램이 시작되는 시점에 초기화된다.
- main()메소드가 실행되기 전에 초기화되므로 main()메소드 실행시 별도의 선언없이
변수를 사용할 수 있다.

접근방법
- 클래스 내부 : 변수명으로 접근할 수 있다. 일반적인 멤버변수와 동일하게 접근한다.
- 클래스 외부 : 
	1. 인스턴스 변수를 통해 접근한다. (권장사항 아님)
	2. 인스턴스 생성없이 클래스명으로 직접 접근한다. (권장사항)
- 메소드 앞에 static을 붙이면 정적메소드가 된다. 정적변수와 모든 특성이 동일하다. */
class MyStatic {
	/* 멤버변수 : 인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한 후 참조변수를
	통해 접근해야 한다. 즉, 일반적인 멤버변수는 인스턴스를 생성하는 시점에 메모리에
	로드된다. (멤버변수는 Stack영역에, 인스턴스는 Heap영역에 생성된다.) */
	int instanceVar;
	/* 정적멤버변수 : 객체를 생성할 필요없이 클래스명으로 바로 접근할 수 있다. JVM
	(자바가상머신)이 프로그램을 시작할 때 이미 메모리(Method영역)에 로드된다. */
	...
	/* 정적멤버메소드 : 정적메소드 내부에서는 인스턴스형 멤버에는 접근할 수 없다. 정적
	멤버만 접근할 수 있다. 
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		/* 일반적인 멤버변수에 접근하는 방법 : MyStatic클래스의 인스턴스를 생성한
		후 참조변수를 통해 접근한다. */
		...
		Java에서는 권장하지 않는다.
		(※ 참조변수를 통해 접근할 거면, 굳이 정젹변수를 사용할 이유가 없기 때문이다.) */
		myStatic.staticVar = 300;
		System.out.println("myStatic.staticVar=" + myStatic.staticVar);
		
		//////////////////////////////////////////////////
		
		/* 2개의 인스턴스를 생성한다. 메모리(Heap영역)에는 ms1, ms2의 인스턴스가
		각각 생성된다. */
		...
		/* 하지만 정적변수는 메모리(Method영역)에 딱 하나만 생성되므로, 어떤
		인스턴스 변수로 접근하든 동일한 메모리가 된다. 따라서 값을 공유하게 되므로
		동일한 결과를 출력한다. */
		ms1.staticVar = 800;
		ms2.staticVar = 900;
		...
		/* 결과를 보듯 입력시에는 각각의 참조변수를 사용했지만 결국 하나의 정적변수에
		값을 입력한 결과가 되므로, 정적변수에 접근시에는 참조변수를 사용하지 말고,
		클래스 명을 사용하는 것이 좋다.
		MyStatic.staticVar = 800;
		MyStatic.staticVar = 900;
		이렇게 작성하면 명시적으로 같은 변수임을 한 눈에 알 수 있다. */
		
		/* 정적멤버메소드의 접근 방법은 정적변수와 동일하다. 참조변수를 통해서는
		인스턴스형, 정적형 둘 다 접근할 수 있다. */
		myStatic.instanceMethod();
		myStatic.staticMethod(); // 권장사항 아님
		
		/* 클래스명을 통한 접근은 정적메소드만 가능하다. 인스턴스메소드는 인스턴스
		참조변수를 통해서만 접근할 수 있다. */
//		MyStatic.instanceMethod(); // 에러발생
		MyStatic.staticMethod(); // 권장사항
		/* 결론 : 일반적인 인스턴스형 멤버는 객체의 참조변수를 통해 접근한다. 정적
		(static)멤버는 클래스명을 통해 인스턴스 생성없이 접근한다. */
	}
