...
- 하지만 개발자가 정의한 클래스라면 hashCode(), equals()메소드를 적절히 오버라이딩해야만
중복을 제거할 수 있다.
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		// 객체저장 : add()메소드를 통해 저장하고, 성공하면 true를 반환한다.
		...
		// size()를 통해 저장된 객체 수를 알 수 있다.
		System.out.println("[중복 저장 전 객체 수]:" + set.size()); // 5개
		
		/* 기본클래스의 객체 중복저장 : 기본클래스인 경우 별도의 오버라이딩 없이도 중복이
		제거된다. 따라서 아래의 add()메소드는 false를 반환한다. */
		...
		while (itr.hasNext()) {
			...
			다운캐스팅하여 출력한다. 만약 다운캐스팅을 하지 않으면 자식객체로의 자식멤버에
		...
		/* 객체삭제 : set컬렉션은 인덱스가 없으므로 객체의 참조값을 통해서만 삭제할 수
		있다. */
