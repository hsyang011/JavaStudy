...
//		Vector<String> list = new Vector<String>();
		...
		add(인덱스, 인스턴스) : 데이터 저장시 인덱스를 직접 부여한다. 단, 인덱스를
		건너뛰면 에러가 발생하므로 주의해야 한다.
		만약 특정 인덱스에 이미 저장된 데이터가 있다면 삽입(끼워넣기)이 된다. */
		list.add("소녀시대");
		list.add("빅뱅");
		list.add("트와이스");
		list.add(3, "워너원");
//		list.add(5, "소방차ㅋㅋ"); // 에러발생됨
		...
		/* 중복저장 : List는 배열의 특성을 가지므로 데이터를 중복저장할 수 있다. 동일한
		데이터라도 index로 구분할 수 있기 때문이다. add()는 추가에 성공할 경우 true를
		반환한다. */
		System.out.println(list.add("트와이스") ? "중복저장됨" : "중복저장안됨");
		System.out.println("중복 저장 후 객체 수:" + list.size());
		
		/* 컬렉션 출력시에는 아래 3가지 방법을 사용한다.
		방법1 : 일반 for문 사용. 이때는 인덱스를 통해 접근해야 하므로 get()메소드를
		사용한다. */
		System.out.println("\n[일반 for문 사용]");
		...
		/* 방법2 : 확장 for문의 경우 인덱스 없이 전체 원소를 반복해주므로 좀 더 간단한
		표현이 가능하다. 따라서 가장 많이 사용된다. */
		System.out.println("\n[확장 for문 사용]");
		for (String obj : list) {
		...
		/* 방법3 : Iterator사용
		1. 컬렉션에 저장된 내용을 Iterator객체에게 알려주기 위해 인스턴스를 생성한다.
		2. hasNext()로 출력할 인스턴스가 있는지 검사한다. 만약 인스턴스가 더이상 없다면
		false를 반환한다.
		...
		/* 수정하기(덮어쓰기) : 기존의 내용을 변경한다. 즉 기존의 내용은 삭제한 후
		새로운 내용으로 갱신한다. */
		...
		/* 삽입하기 : add(삽입할 인덱스, 인스턴스) 해당 인덱스에 인스턴스를 삽입한다.
		...
		/* 포함여부확인
		contains(객체) : 특정 인스턴스가 저장되어 있는지 확인할 때 사용한다. 존재한다면
		true를 반환한다. */
		...
		방법1 : 인덱스로 삭제
			...
			인스턴스를 반환한다. 삭제 후에는 인덱스가 자동으로 부여된다. 삭제 성공시 해당
		...
		이 경우에는 삭제에 성공한 경우 true가 반환된다. 즉 boolean타입의 반환값을
		가진다. */
		...
		/* 전체삭제 : 아래 2개의 메소드를 통해 컬렉션에 저장된 전체 데이터를 삭제할 수
		있다. */
		list.removeAll(list);
		list.clear();
