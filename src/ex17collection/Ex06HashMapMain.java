...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/* 객체저장 : 객체저장시 기존에 저장된 동일한 Key값이 존재하면 저장된 객체가
...
//		map.put("age", number);
		...
		/* 출력하기
		1. 키값을 알고 있을 때 : get(Key값)으로 출력한다. */
		System.out.println("키값을 알 때:" + map.get("name"));
		
		/* 2. 키값을 모를 때 혹은 키 값이 너무 많아서 하나씩 명시하기 힘들 때 :
		keySet()을 통해 전체 Key를 Set컬렉션으로 얻어온다. 해당 반환값을 확장for문을
		통해 반복하고, 이를 통해 Value를 추출할 수 있다. */
		...
		/* 3. 이터레이터 사용 : 이터레이터를 통해 출력할 때도 keySet()을 통해 key를
		먼저 얻어온 후 반복출력한다. */
		...
		/* value만 얻어올 때는 values()메소드를 통해 값을 추출할 수 있다. 그리고
		확장for문을 통해 출력한다. */
		...
		/* 객체의 존재유무 확인 : Map컬렉션은 Key와 Value가 있으므로 메소드도 2개로
		나눠져있다. */
